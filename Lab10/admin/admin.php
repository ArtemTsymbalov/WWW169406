...
function ListaPodstron()
{
    ...
    if ($result->num_rows > 0) {
        echo '<table border="1">';
        echo '<tr><th>ID</th><th>Title</th><th>Опції</th></tr>';
        while ($row = $result->fetch_assoc()) {
            echo '<tr><td>' . $row['id'] . '</td><td>' . $row['page_title'] . '</td>';
            echo '<td><a href="?action=edit&id=' . $row['id'] . '">edit</a> | ';
            echo '<a href="?action=delete&id=' . $row['id'] . '" onclick="return confirm(\'Are you sure you want to delete this subpage?\')">Delete</a></td></tr>';
        }
        echo '</table>';
    } else {
