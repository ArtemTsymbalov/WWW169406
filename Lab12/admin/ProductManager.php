...
class ProductManager {
    ...
    public function edytujProdukt($productId) {
        ...
        if ($result->num_rows > 0) {
            ...
            $categories = $this->conn->query($categoriesQuery);
    
            echo '<form method="post" action="?action=edit_product&id=' . $productId . '" enctype="multipart/form-data">
                <h2>Edit Product</h2>
                ...
                <label>Description:</label><br>
                <textarea name="description" rows="5" required>' . htmlspecialchars($product['description']) . '</textarea><br><br>
            ...
            echo '</select><br><br>
                ...
                <label>Net Price:</label><br>
                <input type="number" name="net_price" step="0.01" value="' . $product['net_price'] . '" required><br><br>
                
                <label>VAT Rate (%):</label><br>
                <input type="number" name="vat_rate" step="0.01" value="' . $product['vat_rate'] . '" required><br><br>
                
                <label>Stock Quantity:</label><br>
                <input type="number" name="stock_quantity" value="' . $product['stock_quantity'] . '" required><br><br>
                
                <label>Dimensions (LxWxH cm):</label><br>
                <input type="text" name="dimensions" value="' . htmlspecialchars($product['dimensions']) . '"><br><br>
                
                <label>Expiry Date:</label><br>
                <input type="date" name="expiry_date" value="' . $product['expiry_date'] . '"><br><br>
                
                <label>Product Image:</label><br>
                <input type="file" name="product_image"><br><br>
                
                <label>Availability:</label>
                <input type="checkbox" name="availability_status" value="1" ' . ($product['availability_status'] ? 'checked' : '') . '><br><br>
                
                <input type="submit" name="submit_edit_product" value="Update Product">
            </form>';
        } else {
    ...
    private function handleProductEdit($productId) {
        $title = $this->conn->real_escape_string($_POST['title']);
        $description = $this->conn->real_escape_string($_POST['description']);
        $category_id = (int)$_POST['category_id'];
        $net_price = (float)$_POST['net_price'];
        ...
        $query = "UPDATE products 
                  SET title = ?, description = ?, category_id = ?, net_price = ?, vat_rate = ?, 
                      stock_quantity = ?, dimensions = ?, expiry_date = $expiry_date, 
                      image_url = IF(? = '', image_url, ?), availability_status = ?
                  WHERE id = ?";
                  
        $stmt = $this->conn->prepare($query);
        $stmt->bind_param("ssiiiisssii", $title, $description, $category_id, $net_price, 
                          $vat_rate, $stock_quantity, $dimensions, $image_url, 
                          $image_url, $availability_status, $productId);
